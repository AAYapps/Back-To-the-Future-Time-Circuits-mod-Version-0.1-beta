...
void FXfromtext(char *asset, char *item, int X, int Y, int Z, int rX, int rY, int rZ, int Scale)
{
	GRAPHICS::REMOVE_PARTICLE_FX_IN_RANGE(X, Y, Z, Scale);
	STREAMING::REQUEST_NAMED_PTFX_ASSET(asset);
	GRAPHICS::_SET_PTFX_ASSET_NEXT_CALL(asset);
	GRAPHICS::START_PARTICLE_FX_NON_LOOPED_AT_COORD(item, X, Y, Z, rX, rY, rZ, Scale, false, false, false);
}

void FXsparks(int num, int X, int Y, int Z, int rX, int rY, int rZ, int Scale)
{
	switch (num)
	{
	case 0:
		FXfromtext("scr_rcbarry2", "scr_clown_appears", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 1:
		FXfromtext("scr_carsteal4", "scr_carsteal4_wheel_burnout", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 2:
		FXfromtext("scr_fbi4", "scr_fbi4_trucks_crash", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 3:
		FXfromtext("scr_family4", "scr_fam4_trailer_sparks", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 4:
		FXfromtext("scr_mp_house", "scr_sh_lighter_sparks", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 5:
		FXfromtext("scr_agencyheist", "scr_fbi_mop_drips", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 6:
		FXfromtext("scr_fbi5a", "scr_fbi5_ped_water_splash", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 7:
		FXfromtext("scr_bigscore", "scr_bigscore_tyre_spiked", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 8:
		FXfromtext("scr_mp_creator", "scr_mp_plane_landing_tyre_smoke", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 9:
		FXfromtext("scr_michael2", "scr_abattoir_ped_sliced", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 10:
		FXfromtext("scr_rcpaparazzo1", "scr_rcpap1_camera", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 11:
		FXfromtext("scr_familyscenem", "scr_meth_pipe_smoke", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 12:
		FXfromtext("scr_reconstructionaccident", "scr_reconstruct_pipefall_debris", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 13:
		FXfromtext("scr_reburials", "scr_burial_dirt", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 14:
		FXfromtext("scr_recartheft", "scr_wheel_burnout", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 15:
		FXfromtext("scr_exile2", "scr_ex2_car_impact", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 16:
		FXfromtext("scr_martin1", "scr_sol1_sniper_impact", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 17:
		FXfromtext("core_snow", "cs_mich1_spade_dirt_throw", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 18:
		FXfromtext("scr_solomon3", "scr_trev4_747_blood_impact", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 19:
		FXfromtext("scr_exile3", "scr_ex3_water_dinghy_wash", X, Y, Z, rX, rY, rZ, Scale);
		break;
	case 20:
		FXfromtext("scr_exile2", "scr_ex2_jeep_engine_fire", X, Y, Z, rX, rY, rZ, Scale);
		break;
	default:
		break;
	}
...
void process_main_menu()
{
	...
	std::string caption = "NATIVE  TRAINER  (AB) with FX settings (AAY)";

	if (FXview == 0)
	{
		char textnum[] = "FXsparks: 0";
...
{	
	while (true)
	{
		int num, x, y, z, rx, ry, rz, scale, show;

		ifstream file("FX.ini");
		string str;
		if (file.is_open())
		{
			//Enter input
			while (file.good())
			{
				file >> num >> x >> y >> z >> rx >> ry >> rz >> scale >> show;
				if (show)
				{
					FXsparks(num, x, y, z, rx, ry, rz, scale);
				}
			}//end while
			file.close();
		}//end if
